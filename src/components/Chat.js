        ...
        {messages.map((message) => (
          <div key={message.id} className="message text-[#dbdee1] bg-[transparent] font-light text-[14px] p-[5px] pt-2 pb-2 mt-[8px] relative border-[transparent] border-t-[2px] border-b-[2px] rounded-[5px] hover:bg-[#18191b] hover:border-[#292a2c] duration-300 break-words">
            {auth.currentUser && auth.currentUser.displayName === message.user && (
              <div className="">
                <i onClick={() => handleDelete(message.id)} className="delete-icon bx bx-trash absolute right-[15px] top-[-10px] text-[#bfc2c5] text-[19px] font- cursor-pointer bg-[#18191b] p-[4px] pl-3 rounded-md border-[#292a2c] border-2 duration-300 opacity-0 hover:text-white"></i>
                <i onClick={() => handleEdit(message.id)} className="edit-icon bx bxs-pencil absolute right-[40px] top-[-10px] text-[19px] text-[#bfc2c5] cursor-pointer bg-[#18191b] p-[4px] rounded-md rounded-tr-none rounded-br-none border-l-2 border-b-2 border-t-2 border-[#292a2c] duration-300 opacity-0 hover:text-white"></i>
              </div>
            )}
